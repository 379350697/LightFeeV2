...
class CloseRuntime:
    ...
    def _merge_pending_close_leg_evidence(self, pending_close: Any) -> int:
        ...
        record of the executed order identities.  Dropping it orphans that
        evidence and leaves the reconciliation exact-recheck retrying the
        passive zero-fill identities forever, so the executor legs merge
        into the existing reconciliation for the same position instead.
        """
            ...
            for side, legs in (
                ...
                for leg in legs if isinstance(legs, list) else []:
                    ...
                    if existing is not None:
                        # A proved fill upgrades the same identity's stored
                        # zero-fill intent; weaker evidence never downgrades.
                        if (
                    ...
                    merged_per_owner += 1
            # The fallback registration snapshots the position mid-writeback,
            # so its owned segment can miss a leg the same flow already closed
            # (production: long 62/short 0 while both taker legs filled).  The
            # executor's proven close totals upgrade the segment; never down.
