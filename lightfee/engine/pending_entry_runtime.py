...
def apply_pending_entry_hedge_progress(
    ...
    *,
    ...
    if delta > 1e-12:
        ...
        note_observed = getattr(pending, "note_hedge_fill_observed", None)
    ...
    async def _reconcile_pending_state(self, now_ms: int) -> None:
            ...
            if result.long_fill is not None and result.long_fill.quantity > 0:
                if pending.maker_leg == "long":
                    ...
                    if result.long_fill.quantity > pending.hedge_leg_filled:
                        hedge_filled_updated = apply_pending_entry_hedge_progress(
                            pending,
        ...
        for close_id, pending in list(self.ctx.state.pending_closes.items()):
            if pending.long_uncertain or pending.short_uncertain:
                ...
                if pos is None:
                    # The position is already terminal; the only route left
                    # for this close's executed-order evidence is the durable
                    # final billing owner for the same position (never a
                    # partial owner).
                ...
                if result.is_flat:
                    # Merge before resolving: a flat probe settles exposure
                    # but not billing, and this PendingClose may hold the
                    # only executed-order identities the final billing owner
                    # can query.
                    self.ctx._merge_pending_close_leg_evidence(pending)
                    resolved_ids.append(close_id)
