...
class LiveRuntime:
    ...
    def __init__(self, config: AppConfig, venue_adapters: Optional[dict[Venue, VenueAdapter]] = None) -> None:
        ...
        # Private account streams are process-scoped after their first valid
        # activation.  The per-venue sets grow monotonically so a transient
        # entry candidate cannot stop or replace a worker that may still
        # receive late fills or position updates.
        ...
        # Terminal evidence failures rotate one route behind the remaining
        # frontier for the current fair-queue round.  This is deliberately
        # separate from the V1 Local-L2 primary assignment lease.
        self._entry_execution_queue_failures: dict[str, dict[str, object]] = {}
        ...
        # independent from Local-L2 resource ownership, so evidence outcomes
        # cannot silently become a wall-clock L2 blacklist.
        ...
        if self._venue_adapters:
            ...
            self.passive_close_executor = PassiveCloseExecutor(
                ...
                config_overrides={
                    ...
                    "max_zero_fill_cycles": (
                        self.config.strategy.maker_phase_max_zero_fill_cycles
                    ),
                    "maker_cycle_retry_delays_ms": list(
                        self.config.strategy.maker_cycle_retry_delays_ms or []
                    ),
    ...
    def _effective_local_l2_scan_assignment_lease_ttl_ms(self) -> int | None:
        """Return V1's opt-in lease for an unready Local-L2 primary only."""
        strategy = self.config.strategy
        if not bool(strategy.local_l2_scan_assignment_lease_enabled):
            return None
        try:
            ttl_secs = int(strategy.local_l2_scan_assignment_lease_ttl_secs or 0)
        except (TypeError, ValueError):
            return None
        return ttl_secs * 1_000 if ttl_secs > 0 else None

    def _record_entry_execution_queue_failure(
        self,
        candidate,
        *,
        reason: str,
        ...
        """Rotate a terminally rejected route for the current fair-queue round."""
        if reason == "entry_local_l2_waiting_for_primary_tracking":
            return False
        pair_id = self._candidate_pair_id(candidate)
        if not pair_id:
            return False
        previous = self._entry_execution_queue_failures.get(pair_id)
        ...
        failure = {
            ...
            "scope": "entry_execution_queue_round",
        }
        self._entry_execution_queue_failures[pair_id] = failure
        return previous != failure

    def _clear_entry_execution_queue_failure(
        self,
        candidate,
        *,
        ...
        pair_id = self._candidate_pair_id(candidate)
        failure = self._entry_execution_queue_failures.get(pair_id)
        if failure is None:
            return False
        if (
            ...
            return False
        cleared = self._entry_execution_queue_failures.pop(pair_id, None) is not None
        if cleared:
    ...
    def _entry_execution_queue_terminal_failure(reason: str) -> bool:
        ...
        if normalized in {
            ...
            "candidate_final_revalidation_failed",
        }:
            return True
    ...
    def _ordered_entry_execution_queue(self, candidates) -> list:
        ...
        if (
            ...
            for pair_id in present_pair_ids:
                self._entry_execution_queue_failures.pop(pair_id, None)
        return sorted(
            source_candidates,
    ...
    def _select_v1_entry_tracked_scope(self, candidates) -> tuple[list, list]:
        ...
        now_ms = wall_clock_now_ms()
        # Quote/OI/economics outcomes may rotate one route for the current
        # execution queue round, but are not Local-L2 leases.  A successful
        # evidence pass or the end of the round clears this scoped exclusion.
        primary_excluded_pair_ids.update(self._entry_execution_queue_failures)
        ...
        if self._entry_local_l2_effective_enabled():
            ...
            for candidate in source_candidates:
                ...
                # V1 only relinquishes an expired, unready primary when a
                # distinct eligible route can actually use the resource.
                # Otherwise retaining it avoids turning a temporary slow
                # subscription into an empty primary window.
                lease_replacement_available = any(
                    (other_pair_id := self._candidate_pair_id(other))
                    and other_pair_id != pair_id
                    and other_pair_id not in primary_excluded_pair_ids
                    and other_pair_id not in executing_pair_ids
                ...
                if (
                    ...
                    or (
                        lease_expired_without_readiness
                        and lease_replacement_available
                    )
                ):
    ...
    async def _collect_entry_candidate_truth_for_venue(
        self,
        venue: Venue,
        adapter: VenueAdapter,
        symbol: str,
    ) -> dict[str, Any]:
        """Read position and open-order truth for one candidate leg only."""
        venue_name = venue.value
        ...
        transport = getattr(adapter, "_transport", adapter)
        private_state = getattr(transport, "private_ws_state", None)
        ...
        if private_position is not None:
            ...
            if not callable(fetch_position):
                ...
                try:
                    ...
                    if position is None:
                        raise RuntimeError("fetch_position_returned_none")
                    positions.append(
                        self._recovery_ledger_position_payload(
                            position,
                            venue_name=venue_name,
        ...
        return {
            "venue": venue_name,
            "truth_supported": bool(probe_evidence),
            "truth_available": not errors,
            "complete": complete,
            "positions": positions,
            "open_orders": open_orders,
            "probe_evidence": probe_evidence,
            "errors": errors,
            "truth_probe_count": len(probe_evidence),
            "started_at_ms": started_at_ms,
            "finished_at_ms": finished_at_ms,
        }
    ...
    async def _entry_account_truth_target_receipts(
        ...
        for venue in required_venues:
            ...
            cached = self._entry_account_truth_venue_receipts.get(receipt_key)
    ...
    def _snapshot_target_only_retention_reason(
        ...
        *,
        ...
        market_max_age_ms = int(
            self.config.runtime.sidecar_market_observation_max_age_ms
            or max_age_ms
        )
        if market_age_ms <= market_max_age_ms:
            return ""
        # ``acquisition_mode`` is whole-snapshot provenance. It can describe
        # an unrelated venue/symbol fallback while this pair still has current
        # per-leg evidence. The candidate freshness and final BBO/L2/OI gates
        # below own that scoped decision; broad provenance must not erase it.
    ...
    def _entry_consumer_frontier(self, candidates):
        """Keep ranking intact until each candidate is checked in turn."""
        return list(candidates)

    def _current_sidecar_snapshot_generation(self) -> tuple[object, bool]:
        path = self.config.runtime.sidecar_snapshot_path
        try:
            stat = Path(path).stat()
            return ("sidecar", stat.st_mtime_ns, stat.st_size), True
        except OSError:
            return ("missing",), False

    async def _sidecar_snapshot_for_tick(self):
        # V1 reads one atomically replaced sidecar snapshot on every scan.
        # A missing or malformed file is handled by normal snapshot freshness;
        # it never falls back to a second entry-specific data plane.
    ...
    async def _run_ranked_candidate_entry_flow(
        ...
        *,
        ...
        # It never creates a time-based Local-L2 ownership exclusion.
        candidates = self._ordered_entry_execution_queue(candidates)
        ...
        fresh_evidence_pair_ids: set[str] = set()
        ...
        for source_candidate in candidates:
            if remaining_slots <= 0 or not can_enter_new_positions(self.state):
                break
            checked += 1
            candidate_rows = await self._filter_candidates_supported_by_venue_catalog(
                [source_candidate]
            )
            candidate_rows = self._filter_candidates_by_entry_admission(
                candidate_rows,
                now_ms=wall_clock_now_ms(),
                stage="ranked_candidate",
            )
            candidate_rows = await self._filter_candidates_by_entry_balance_admission(
                candidate_rows,
                now_ms=wall_clock_now_ms(),
                stage="ranked_candidate",
            )
            ...
            if isinstance(quote_result, BaseException):
                ...
                self.journal.append(
                    "runtime.entry_candidate_revalidation_failed",
                    {
                        "pair_id": self._candidate_pair_id(candidate),
                        "domain": "quote",
                        "error": f"{type(quote_result).__name__}: {quote_result}"[:240],
                        "ts_ms": wall_clock_now_ms(),
                    },
                )
                continue
            ...
            candidate_rows = self._reprice_entry_candidates_for_selection(
                candidate_rows,
            ...
            selected_deadline = (
                asyncio.get_running_loop().time() + submit_deadline_ms / 1_000.0
                if submit_deadline_ms > 0
                else None
            )
        ...
        # Feed terminal evidence outcomes back into the *next* queue round.
        # The selection scope consumes this only as a fair rotation; it is
        # intentionally not connected to the V1 90s Local-L2 lease.
        for candidate in candidates:
            pair_id = self._candidate_pair_id(candidate)
            reason = candidate_blockers.get(pair_id)
            if reason and self._entry_execution_queue_terminal_failure(reason):
                self._record_entry_execution_queue_failure(
                    candidate,
                    reason=reason,
                    now_ms=wall_clock_now_ms(),
                )
            elif pair_id in fresh_evidence_pair_ids:
                self._clear_entry_execution_queue_failure(candidate)
        ...
        if dispatched == 0:
            ...
            self.journal.append(
                "scan.no_entry_ranked_candidates",
                {
                    "reason": last_reason,
    ...
    async def tick(self) -> None:
        ...
        def usable_entry_snapshot(candidate) -> bool:
            return has_usable_funding_payload(candidate)
        ...
        if freshness == SnapshotFreshness.MISSING:
            ...
            payload = {
                "ts_ms": now_ms,
                "blocking_stage": "sidecar_snapshot",
        ...
        snapshot_published_at_ms = self.market_data_runtime._snapshot_publication_at_ms(
            snapshot
        )
        snapshot_publish_age_ms = (
            max(now_ms - snapshot_published_at_ms, 0)
            if snapshot_published_at_ms > 0
            else None
        )
        snapshot_publish_warn_ms = int(
            self.config.runtime.sidecar_snapshot_publish_warn_ms
        )
        snapshot_publish_slow = bool(
            snapshot_publish_age_ms is not None
            and snapshot_publish_age_ms > snapshot_publish_warn_ms
            and snapshot_publish_age_ms <= max_age
            and freshness in {SnapshotFreshness.FRESH, SnapshotFreshness.DEGRADED}
        )
        self.state.last_scan = {
            "ts_ms": now_ms,
            "snapshot_freshness": freshness.value if hasattr(freshness, "value") else str(freshness),
            "snapshot_publication_age_ms": snapshot_publish_age_ms,
            "snapshot_publication_warn_ms": snapshot_publish_warn_ms,
            "snapshot_publication_availability_max_age_ms": max_age,
            "snapshot_publication_slow": snapshot_publish_slow,
        ...
        if snapshot_publish_slow:
            self._append_runtime_diagnostic_event(
                "runtime.snapshot_publish_slow",
                {
                    "ts_ms": now_ms,
                    "owner": "sidecar",
                    "classification": "producer_slo_warning",
                    "blocking": False,
                    "snapshot_publish_age_ms": snapshot_publish_age_ms,
                    "snapshot_publish_warn_ms": snapshot_publish_warn_ms,
                    "snapshot_availability_max_age_ms": max_age,
                    "snapshot_freshness": (
                        freshness.value
                ...
                now_ms=now_ms,
                key_parts=("global_sidecar_publication",),
                interval_ms=max(snapshot_publish_warn_ms, 30_000),
            )
        self._schedule_current_state_snapshot_export(self._export_state, now_ms)
        if freshness == SnapshotFreshness.LAST_GOOD_FALLBACK:
            self.journal.append(
        ...
        if can_enter_new_positions(self.state) and self.entry_executor is not None:
            ...
            await self._run_ranked_candidate_entry_flow(
                tradeable,
                snapshot=snapshot,
                price_hints=price_hints,
    ...
    def _ensure_private_ws_started(self, now_ms: int) -> None:
        """Maintain private account streams without binding them to candidate churn.

        V1 starts one private worker and lets that worker own reconnects.  V2
        may discover a new valid symbol after startup, so it extends the
        worker's symbol mapping/subscriptions in place.  Removing a candidate
        never stops a worker: private truth for a late fill or recovered
        position must remain available until explicit service shutdown.
        """
        if self.config.runtime.mode == "paper":
            return
        ...
        for venue, adapter in self._venue_adapters.items():
            ...
            # No current candidate is not a transport lifecycle event.  Keep
            # an already running account stream alive; the explicit shutdown
            # path remains its sole normal stop owner.
            if not symbols:
                continue
            ...
            previous_symbols = self._private_ws_symbols.setdefault(venue, set())
            additions = symbols - previous_symbols
            if not additions:
                continue

            update_symbols = getattr(transport, "update_private_ws_symbols", None)
            if not callable(update_symbols):
                # Every production VenueTransport implements this method.  Do
                # not fall back to stop/start for a partial or test adapter:
                # replacement is precisely the unsafe lifecycle behavior this
                # owner prevents.
                logger.warning(
                    "private WS symbol update unavailable; retaining existing worker venue=%s additions=%d",
                    venue.value,
                    len(additions),
                )
                continue
            update_symbols(sorted(additions))
            previous_symbols.update(additions)
    ...
    def _reprice_entry_candidates_for_selection(
        ...
        *,
        ...
        for source_candidate in candidates:
            ...
            live_notional_cap = 0.0
            if self.config.runtime.mode == "live":
                configured_cap = max(
                    float(
                        getattr(
                            self.config.strategy,
                            "entry_notional_cap_quote",
                            0.0,
                        )
                        or 0.0
                    ),
                ...
                configured_live_cap = max(
                    float(
                        getattr(
                            self.config.strategy,
                            "live_entry_notional_cap_quote",
                            0.0,
                        )
                        or 0.0
                    ),
                ...
                live_notional_cap = (
                    min(configured_cap, configured_live_cap)
                    if configured_live_cap > 0.0
                    else configured_cap
                )

            def _cap_to_live_notional(quantity: float, price: float) -> float:
                if live_notional_cap <= 0.0 or price <= 0.0:
                    return quantity
                return min(quantity, live_notional_cap / price)

            aligned_quantity = _cap_to_live_notional(
                requested_quantity,
                conservative_max_price,
            )
            if has_explicit_target_quantity and common_step > 0.0:
                aligned_quantity = _align_base_quantity_down(
                    aligned_quantity,
                    common_step,
                )
            ...
            candidate.entry_max_leg_notional_quote = (
                final_quantity * conservative_max_price
            )
            candidate.entry_notional_quote = final_quantity * (
                long_ask + short_bid
            ) / 2.0
            ...
            decision = revalidator.revalidate_before_first_leg(
                ...
                required_base_quantity=final_quantity,
                l2_vwap_complete=(
    ...
    def _final_quote_lease_reason(
        ...
        return self.entry_dispatch_runtime._final_quote_lease_reason(
            candidate,
            lease,
            now_ms,
        )
