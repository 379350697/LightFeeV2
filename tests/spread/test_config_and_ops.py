...
def test_spread_reversion_config_defaults_are_disabled_and_small() -> None:
    ...
    assert not hasattr(runtime, "spread_sidecar_source_mode")
    assert not hasattr(runtime, "spread_sidecar_direct_fetch_enabled")
    ...
    assert not hasattr(strategy, "spread_paper_research_manifest_path")
    ...
    assert not hasattr(PersistenceConfig(), "spread_paper_rollback_anchor_path")
    assert PersistenceConfig().spread_paper_event_log_hard_max_bytes == 67_108_864


def test_loads_single_process_spread_paper_config(tmp_path) -> None:
...
""",
    ...
    assert not hasattr(config.runtime, "spread_sidecar_source_mode")
    assert not hasattr(config.runtime, "spread_sidecar_direct_fetch_enabled")
    ...
    assert not hasattr(config.strategy, "spread_paper_research_manifest_path")
    ...
    assert not hasattr(config.persistence, "spread_paper_rollback_anchor_path")
