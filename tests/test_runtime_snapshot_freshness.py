...
def _isolate_non_canary_snapshot_stages(monkeypatch):
    """Isolate account and catalog concerns from snapshot freshness tests."""
...
def test_live_runtime_has_only_the_sidecar_snapshot_funding_input() -> None:
    from lightfee.sidecar.service import SidecarService

    config = AppConfig(symbols=["BTCUSDT"])
    runtime = LiveRuntime(config)

    assert "opportunity_input_mode" not in config.runtime.__dataclass_fields__
    assert not hasattr(runtime, "_in_process_entry_snapshot_for_tick")
    assert not hasattr(runtime, "_ensure_in_process_entry_source")
    assert not hasattr(SidecarService, "refresh_in_process_entry")
...
def test_entry_oi_refresher_has_no_prewarm_or_durable_state() -> None:
    refresher = EntryOpenInterestRefresher(targeted_budget_s=1.0)

    assert not hasattr(refresher, "_prewarm_inflight_keys")
    assert not hasattr(refresher, "_durable_store")
    assert "priority" not in EntryOpenInterestRefresher.refresh_open_interest.__annotations__
...
async def test_entry_oi_write_through_uses_completion_clock_for_post_start_receipt(
    ...
    await refresher.close()
    restarted = EntryOpenInterestRefresher(targeted_budget_s=1.0)
    assert restarted.cached_open_interest(
        "binance", "BTCUSDT", now_ms=completion_ms + 1
    ) is None
...
@pytest.mark.asyncio
async def test_entry_oi_per_venue_limit_does_not_starve_an_unrelated_venue():
    refresher = EntryOpenInterestRefresher(targeted_budget_s=1.0)
...
@pytest.mark.asyncio
async def test_runtime_loads_one_atomic_sidecar_snapshot_for_each_tick(
    tmp_path,
    monkeypatch,
):
    config = AppConfig(
    ...
    monkeypatch.setattr("lightfee.engine.runtime.load_snapshot", load_generation)
...
def test_live_runtime_does_not_schedule_entry_evidence_prewarm(tmp_path):
    config = AppConfig(
        runtime=RuntimeConfig(
            mode="paper",
    ...
    runtime = LiveRuntime(config)
    assert not hasattr(runtime, "_entry_evidence_prewarm_task")
    assert not hasattr(runtime, "_schedule_entry_evidence_prewarm")
...
def test_frontier_reprice_is_stable_immutable_and_uses_configured_size(tmp_path):
    ...
    assert first[0].entry_target_quantity == second[0].entry_target_quantity
    assert first[0].entry_max_leg_notional_quote <= 50.0 + 1e-9
...
def test_frontier_reprice_uses_common_grid_before_v1_priority_ranking(tmp_path):
    ...
    assert by_symbol["COARSEUSDT"].entry_target_quantity == pytest.approx(0.4)
    assert by_symbol["FINEUSDT"].entry_target_quantity == pytest.approx(0.47)
    assert (
...
def test_frontier_reprice_preserves_original_minimum_failure_reason(
    tmp_path,
):
    config = AppConfig(
        runtime=RuntimeConfig(mode="paper"),
