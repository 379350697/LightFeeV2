...
| CL-153 | deployed-awaiting-verification (implemented but not closed) | `935ef683` | Gate dual-mode `set_leverage` production-shape regression: single-mode dict, array shape with the `lever` alias, mismatch/malformed/empty rejections, and the unchanged dual-retry path; RED on the pre-fix tree reproducing the exact production raw error | Deployed `2026-09-16 20:02 CST` (`6610b4d4`). 审核保留意见：数组形状来自 GET positions + 错误推断（官方 POST 文档 200 schema 为单个 Position），未保存生产 POST 原始响应；dual-side retry 的数组/非目标 side 反例未覆盖。自然验证：一条真实 gate 腿 entry 记录 `order.entry_leverage_ready`（含形状派生的 position_mode 标签）且窗口无 `entry_leverage_unavailable`，验证前不得称 closed。 | [2026-09-16](daily/2026-09-16.md#cl-153--gate-双向模式-set_leverage-校验把成功当失败-dual-mode-leverage-verification) |
| CL-154 | root-cause-confirmed (circuit breaker; deployed `6610b4d4` is DEFECTIVE — revert deployed) | `69da782e` reverted | — | **implemented but not closed.** 2026-09-16 深夜审核发现已部署 P1：merge 按 position_id 广播给合法并存的 partial+final 双 owner，并把两者不可变 `owned_close_quantities` 提升为 PendingClose 总量（审核者真实路径反例复现 62/62×2 + 相同 legs）。触发协议第 9 节熔断：revert 已部署，重做 owner/证据归属分析后再 v2。原验收目标（结算两笔现存 LSK 债务）未完成，不得改写范围。 | [2026-09-16](daily/2026-09-16.md#2026-09-16-深夜--闭环审核不通过cl-154-触发修复循环熔断) |
| CL-155 | local-green (deployed `6610b4d4` 已含不完整版本) | `6610b4d4`+ | Warning-transition V1-parity regression: persistent degraded condition journals `risk.warning_triggered` once on entry, `risk.warning_cleared` once on exit, re-degrade re-arms once; RED on the pre-fix tree | **implemented but not closed**：审核指出 V1 warning_active 还要求 `warning_line_enabled`（risk.rs:382-384），已部署版本缺该条件；另"一腿快照长期缺失"的根因未修（仅事件去重）。补齐 parity 后重新部署；自然验证待一条 degraded position 产生一对 triggered/cleared。 | [2026-09-16](daily/2026-09-16.md#cl-155--风险警告按-tick-刷屏v1-转换语义漂移-per-tick-risk-warning-storm) |
