...
| CL-147 | root-cause-confirmed | `cfbf4134` | Bitget Classic/UTA unified historical-close discovery; executed-vs-submitted quantity, close ownership, official trade-side variants, explicit zero execution/fee, cursor pagination, exact order-status recheck, and real `CloseRuntime → BitgetAdapter → VenueTransport` regression; first deployment exposed a live `null` history collection shape; follow-up parser normalization regression passes; close profile `494 passed`; full validation profile `10/10` passed | `cfbf4134` is deployed and exchange truth is high-confidence flat/no-order, but four natural Bitget history queries returned `history_query_error: Bitget order history rows are malformed` because an empty collection was JSON `null`. Hotfix is awaiting deployment; after it is live, observe the same debts read-only and require `no_candidate`/exact evidence outcomes. No order or cancel should be forced. | [2026-09-05](daily/2026-09-05.md#bitget-historical-close-discovery-is-now-wired-to-the-unified-adapter-contract) |
