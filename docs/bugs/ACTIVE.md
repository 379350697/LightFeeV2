...
- Last production SHA checked: `6610b4d4`
- Checked on: `2026-09-16`. CL-153/154/155 batch (`935ef683` gate dual-mode
  set-leverage, `69da782e` executor leg evidence merge, `6610b4d4` warning
  transition parity) was deployed over the Tailscale route at 20:02 CST in a
  verified complete-flat/no-order window (seven-venue truth high-confidence
  flat, zero local positions/pendings before restart). Manifest regenerated
  over the whole tree (431 files), all critical hashes verified, `.deploy_version`
  set to `6610b4d4…`, both services restarted as singletons (strict singleton
  pass after the checker's own SSH wrapper self-match was excluded; the
  transient VIOLATION lines were the deploy shell's command line, not
  processes). Post-restart: lifecycle/risk `running`, NRestarts=0, seven
  journal ERRORs are the old process's shutdown timeouts only. Deployment
  acceptance: 0 critical / 1 warning (`pending_close_owner_present`, 4
  owners — two by-design operator records plus the two CL-154-affected LSK
  debts, which retain evidence lost before this fix and keep their warning-only
  retry loop; see the CL-154 row). No order was submitted or cancelled by
  deployment checks. Remaining scenario proof: a natural gate-leg entry
  proving `order.entry_leverage_ready` on the dual-mode account (CL-153), a
  future fallback close settling through the merge boundary (CL-154), and a
  degraded position journaling one triggered/cleared pair (CL-155).
- Prior checked deployment: `2be127cc`
...
- Checked on: `2026-09-15`. Runtime fix `3a50dcfb` (split-close complement
  ...
  SSH in a verified complete-flat/no-order window after discarding server-side
  file drift that matched the recorded `b6481a37` content byte-for-byte; all
  manifest hashes matched and both services restarted as singletons. Seven
  seconds after restart, `entry-1788537122325-ONGUSDT` reactivated under
  `binance_split_close_complement` and reconciled in the same second: the long
  leg merged the stored 203 partial (fee now proven from its own userTrades,
  `0.00386309`) with the discovered 49 complement (`3274923320`,
  `0.00232995`), the zero-quantity short lookup no longer failed the short
  recheck, and the billing terminalized `final` (exit fee `0.01692484`).
  Owners fell 5 → 3: the remaining records are the Aster terminal miss
  (`1788403990217-CYSUSDT`, one repaired re-scan already ran), the
  `1788537122325` partial (operator import by design), and the
  `1789091700691` active retrier (transient two-leg flat-truth probe misses
  on backoff; both venue probes verified working read-only). No order was
  submitted or cancelled by verification.
- Checked on: `2026-09-14`. CL-150 (`27981205` Binance execution-window
  discovery + `b6481a37` one-scan reactivation guard) was fast-forwarded over
  Tailscale SSH with a whole-tree regenerated manifest; `.deploy_version` set to
  `b6481a3` and all critical manifest hashes verified. An IOSTUSDT position
  opened at the restart boundary and was recovered by startup reconciliation
  (`runtime.reconciling` → `force_reconcile_complete`, position retained and
  later closed normally). Eight seconds after restart both Binance-leg legacy
  `no_candidate` debts reactivated under `binance_execution_window_anchor_repair`:
  `entry-1788583981589-ONGUSDT` reconciled in the same second with complete
  venue-reconciled evidence, and `entry-1788537122325-ONGUSDT` re-scanned once,
  terminalizing under the distinct `binance_user_trades_no_candidate` (split
  203+49 close — operator-import by design). Reconciliation owners fell 6 → 5;
  exactly one reactivation per debt since restart (the one-scan guard held).
  The 5-minute health timer now emits compact output (≈8 lines per fire versus
  ≈1,370-line JSON dumps that had accumulated 829 MB over 9.5 days). Singleton
  checks passed (sidecar 1, live 1, snapshot writers 0). No order was submitted
  or cancelled by verification.
- Prior checked deployment: `3be8d703`
...
| CL-153 | deployed-awaiting-verification | `935ef683` | Gate dual-mode `set_leverage` production-shape regression: single-mode dict, dual-mode `Array<Position>` with the `lever` alias, mismatch/malformed/empty rejections, and the unchanged dual-retry path; RED on the pre-fix tree reproducing the exact production raw error | Deployed `2026-09-16 20:02 CST` (`6610b4d4`). Needs one natural gate-leg entry passing `order.entry_leverage_ready` on the dual-mode account and one funding-capture window completing without `entry_leverage_unavailable`. | [2026-09-16](daily/2026-09-16.md#cl-153--gate-双向模式-set_leverage-校验把成功当失败-dual-mode-leverage-verification) |
| CL-154 | deployed-awaiting-verification | `69da782e` | Executor-leg evidence merge production-path regression: orphan boundary merges executed identities plus the mid-writeback owned-segment upgrade, the next billing cycle settles `exit.reconciled`, no-owner orphan stays a no-op (both-tree green), and unavailable lookups keep the evidence-debt fail-closed; RED on the pre-fix tree (2/3 fail) | Deployed `2026-09-16 20:02 CST` (`6610b4d4`). Scope note: the fix closes the evidence-loss boundary for future fallback closes; the two live debts (`entry-1789498326920-LSKUSDT`, `entry-1789502150664-LSKUSDT`) lost their executed-order identities before this fix and keep their warning-only `close_order_lookup_returned_no_fill` retry — they need a follow-up discovery-grant repair or an operator-evidence import to settle. Needs one natural fallback close settling through the merge boundary. | [2026-09-16](daily/2026-09-16.md#cl-154--fallback-平仓后-executor-成交腿证据被孤儿丢弃-executor-leg-evidence-orphaned) |
| CL-155 | deployed-awaiting-verification | `6610b4d4` | Warning-transition V1-parity regression: persistent degraded condition journals `risk.warning_triggered` once on entry, `risk.warning_cleared` once on exit, re-degrade re-arms once; RED on the pre-fix tree | Deployed `2026-09-16 20:02 CST`. After the deploy, a future degraded position must produce one triggered/cleared pair instead of a per-tick storm. | [2026-09-16](daily/2026-09-16.md#cl-155--风险警告按-tick-刷屏v1-转换语义漂移-per-tick-risk-warning-storm) |
