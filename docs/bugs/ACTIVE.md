...
- Checked on: `2026-09-19 17:0x`. P0 事故遏制部署：`f9fa2463`（CL-156，venue
  ...
  `close_leg_lookup_error` 并安全降级，进程单一连续生命周期，NRestarts=0。
  当前 fail-closed：`RISK_ONLY_WAIT_FOR_TRUTH`（pending entry 与 live truth
  冲突，等 bybit 真值）；交易所遗留 gate 单腿多 LSKUSDT 57 张（≈$25）。
  **阻塞项：用户需在 Bybit 续期/更换 API key，之后重启服务即恢复。**
  未下单未撤单（runtime 全程 fail-closed）。
- Prior checked deployment: `71efecb2`
- Checked on: `2026-09-16 21:0x`. 审核不通过后的遏制部署：`450d76c9`
  revert CL-154 运行时行为（消除 partial/final 双 owner 广播与不可变段
  改写）+ `71efecb2` 补 CL-155 `warning_line_enabled` parity。Tailscale
  通道同步（中途一次 rsync 断连重试后 manifest 全过），`.deploy_version`
  已重写（注意：rsync `--delete` 会删远端 `.deploy_version`，部署脚本须在
  rsync 后写入）。双服务单例、NRestarts=0、重启后零运行时 ERROR；验收
  0 critical / 1 warning（`pending_close_owner_present`，4 owner 不变）。
  期间七所真值保持高置信度全平/无挂单；未下单未撤单。
- Prior checked deployment: `6610b4d4`
...
| CL-154 | closed | `7bc4ff1b`+`9d1e3ca8` | v2 executor-evidence ownership: v2a registration-boundary absorption (final owner born truthful), v2b final-only orphan/flat catch-all merge (partial owners byte-identical — reviewer counterexample RED), v2c one-time discovery grant + Gate unique-window discovery + Gate my_trades `fee` field fix; reviewer counterexample and full-path settlement RED/GREEN | **验收达成（2026-09-22 02:19）**：全部 4 条非 by-design 债离开重试循环——LSK×2 + BRUSDT 经 grant→discovery→`exit.reconciled`（venue-complete，62/62、61/61、24/24），gate-LSK 经 fee 修正+capability 重扫走出 provisional 终态（成交已证）；owners 6→2（by-design 基线）；invalid 循环消失；范围未改写。 | [2026-09-22](daily/2026-09-21.md#2026-09-22--v2c-部署--4-条债全部结清--cl-154-达成原验收目标) |
