...
| CL-153 | local-green | pending | Gate dual-mode `set_leverage` production-shape regression: single-mode dict, dual-mode `Array<Position>` with the `lever` alias, mismatch/malformed/empty rejections, and the unchanged dual-retry path; RED on the pre-fix tree reproducing the exact production raw error | Needs one natural gate-leg entry passing `order.entry_leverage_ready` on a dual-mode account and one funding-capture window completing without `entry_leverage_unavailable`. | [2026-09-16](daily/2026-09-16.md#cl-153--gate-双向模式-set_leverage-校验把成功当失败-dual-mode-leverage-verification) |
| CL-154 | local-green | pending | Executor-leg evidence merge production-path regression: orphan boundary merges executed identities plus the mid-writeback owned-segment upgrade, the next billing cycle settles `exit.reconciled`, no-owner orphan stays a no-op (both-tree green), and unavailable lookups keep the evidence-debt fail-closed; RED on the pre-fix tree (2/3 fail) | Needs the two live LSK debts (`entry-1789498326920-LSKUSDT`, `entry-1789502150664-LSKUSDT`) to settle after deploy without `close_order_lookup_returned_no_fill` re-firing. | [2026-09-16](daily/2026-09-16.md#cl-154--fallback-平仓后-executor-成交腿证据被孤儿丢弃-executor-leg-evidence-orphaned) |
| CL-155 | local-green | pending | Warning-transition V1-parity regression: persistent degraded condition journals `risk.warning_triggered` once on entry, `risk.warning_cleared` once on exit, re-degrade re-arms once; RED on the pre-fix tree | After deploy, a future degraded position must produce one triggered/cleared pair instead of a per-tick storm. | [2026-09-16](daily/2026-09-16.md#cl-155--风险警告按-tick-刷屏v1-转换语义漂移-per-tick-risk-warning-storm) |
