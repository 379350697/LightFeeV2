...
| PE-11 | deterministic hedge admission reject after maker exposure, including account/venue-scoped margin rejects, or an exchange `REJECTED` hedge submit after maker exposure with no accepted hedge order identity | abort through cleanup path; retain fail-closed if cleanup unproven; arm symbol/venue admission cooldown when the reject proves account/venue capacity is exhausted; for generic no-order rejects use `hedge_rejected_after_maker_exposure` evidence-gap cleanup rather than order-truth work | retry same hedge indefinitely, convert a rejected no-order hedge submit into `accepted_order_truth_gap`, or keep submitting/repricing new maker risk through the same one-sided pending entry |
...
| RC-11 | pending entry has positive fill evidence and current live truth shows one owned single leg, including after the pending entry disappears but journal positive-fill/live-conflict evidence remains | create `owned_pending_entry_live_conflict`, block all new entry risk, persist journal position ownership across restart/current-state compaction, route a current single-leg pending conflict through reduce-only cleanup when no open-order truth is present, and release only after account flat plus open-order truth | downgrade to ordinary owned work, orphan drift, local flat, or an owned blocker that never drives cleanup |
...
| PE-11 | `tests/live_harness/test_exchange_admission_incidents.py::test_pending_hedge_hyperliquid_insufficient_margin_reject_aborts_without_retry`, `tests/live_harness/test_exchange_admission_incidents.py::test_pending_hedge_unclassified_reject_after_maker_exposure_aborts_without_retry`, `tests/live_harness/test_exchange_admission_incidents.py::test_recovery_pending_hedge_unclassified_reject_after_maker_exposure_aborts_without_truth_gap_retry`, `tests/test_live_full_closure.py::TestMakerEventLane::test_unhedged_single_leg_pending_blocks_maker_event_reprice`, `tests/test_live_full_closure.py::TestMakerEventLane::test_binance_margin_reject_terminally_blocks_maker_reprice`, `tests/test_live_startup_preflight.py` | covered for known deterministic families, generic exchange-rejected hedge submit after maker exposure, recovery no-order reject truth-gap bypass prevention, maker-event single-leg reprice blocking, and Binance `-2019` admission classification |
...
| PE-16 | `tests/live_harness/test_residual_repair_incident_replay.py::test_exhausted_residual_repair_live_nonzero_repairs_but_untrusted_stays_fail_closed`; `tests/test_pending_entry_v1_semantic_drift.py::test_positive_fill_finalize_cleans_owned_live_single_leg_before_release`; `tests/test_pending_entry_v1_semantic_drift.py::test_positive_fill_owned_live_single_leg_cleanup_failure_retains_pending`; `tests/engine/test_v1_lifecycle_closure_table.py::test_pending_entry_positive_fill_single_leg_live_truth_blocks_open_terminality` | covered for residual untrusted live exposure and 2026-06-14 HOMEUSDT positive-fill single-leg live truth recurrence: current owned single-leg conflicts clean through reduce-only/fresh-flat truth, while cleanup failure retains fail-closed pending work |
...
| RC-11 | `tests/engine/test_recovery_owner_index.py::test_positive_fill_live_conflict_journal_owns_matching_live_position`; `tests/engine/test_recovery_ledger.py::test_journal_positive_fill_conflict_owns_live_position_after_pending_removed`; `tests/test_pending_entry_v1_semantic_drift.py::test_positive_fill_finalize_cleans_owned_live_single_leg_before_release`; `tests/test_pending_entry_v1_semantic_drift.py::test_positive_fill_owned_live_single_leg_cleanup_failure_retains_pending`; `tests/test_diagnose_live.py::test_diagnose_journal_positive_fill_conflict_owns_historical_live_single_leg`; `tests/ops/test_production_health.py::test_current_state_journal_positive_fill_conflict_owns_historical_live_single_leg` | covered for durable journal ownership after pending removal, diagnose/health classification as owned conflict, current pending single-leg reduce-only cleanup success, and cleanup-failure pending retention |
