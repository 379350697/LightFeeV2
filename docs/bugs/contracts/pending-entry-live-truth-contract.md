...
| 2026-06-06 V1 recovery decision core closed-loop | RC-09, RC-10, DG-04, plus DG-01/DG-02 live-artifact preservation | deployed `41ab11e`; production acceptance blocked by exchange-truth evidence gap and post-deploy `risk_only` | keep one pure core as the shared authority for block/clear/lifecycle/entry/diagnose/health; no-work probe gaps are evidence warnings, local work plus missing truth remains risk-only, and live artifacts remain blockers unless exact owner evidence exists. Production verifier/diagnose show the core decision payload is active, but cloud cannot be marked high-confidence green until service-environment exchange truth is available and final lifecycle returns to running/flat/no-open-orders |
