...
and carry exact dispatch blockers into the existing scan diagnostic.  These
are **local-green, not deployed or closed**; the combined full validation
profile passes 1,571 tests and the complete suite passes 4,386 tests with 9
skipped.  The ledger cache is now invalidated against the owner semantics at
...
| [CL-119-terminal-close-debt-entry-gate-semantics](daily/2026-08-22.md#cluster-cl-119-terminal-close-debt-entry-gate-semantics) | local-green; deploy pending | critical | `recovery-ledger`, `recovery-core`, `entry-dispatch`, `v1-parity`, `close-accounting` | `terminal-evidence-debt-permanent-pair-block + active-close-reconciliation-nonblocking-ledger + duplicate-gate-owner + stale-owner-cache` | 2026-08-22 | historical COTI/ONG debt plus post-CL-118 code review at `a5ef64a` | working tree | exact/reversed/different-pair matrix, ledger-build add/transition/remove invalidation, and full runtime dispatch regressions green | RecoveryLedger sole pair gate; V1 global lifecycle split; ledger-build owner fingerprint | Active/unknown/truth-gap exact-pair work blocks; terminal high-confidence-flat debt is warning-visible and nonblocking. Same symbol on a different complete venue pair is allowed. No historical debt/state was changed, and this defect did not account for every threshold-blocked candidate. |
