...
|---|---|---:|---|---|---|---|---|---|---|---|
| [CL-095-passive-close-billing-recovery-symbol-and-joint-entry-grid](daily/2026-08-04.md#cluster-cl-095-passive-close-billing-recovery-symbol-and-joint-entry-grid) | local fixed; deploy pending | critical | `passive-close`, `trade-ledger`, `recovery-owner`, `entry-planner`, `v1-lifecycle` | `passive-close-terminal-bill-missing + identity-less-live-flat-billing-gap + venue-wire-symbol-owner-mismatch + joint-entry-grid-hedge-undercut + okx-minsz-hedge-undercut` | 2026-08-04 | read-only post-deploy lifecycle audit | working tree | local RED/GREEN: recovery wire symbol, joint-grid planner/dispatch, full and identity-less terminal bill, and lifecycle mapping; targeted regression `329 passed` | V1 hedgeability planner and passive-close terminal contract | Filled passive closes write one projected terminal bill with fill evidence; identity-less exchange-proved live-flat cleanup writes one projected provisional bill without a fabricated PnL; known venue wire symbols do not create false orphan ownership; maker/hedge quantities share an executable joint grid and OKX `ctVal × minSz` minimum before maker submission. Snapshot/risk data issues remain fail-closed and are already causally instrumented; no data-source policy was loosened. |
| [CL-094-post-deploy-bankusdt-lifecycle-closure-and-hyperliquid-recovery-observability](daily/2026-08-04.md#cluster-cl-094-post-deploy-bankusdt-lifecycle-closure-and-hyperliquid-recovery-observability) | local fixed; deploy pending | high | `v1-lifecycle-closure`, `entry-admission`, `production-observability`, `v1-parity` | `post-deploy-bankusdt-lifecycle-closure-unmapped + hyperliquid-entry-balance-recovery-evidence` | 2026-08-04 | production BANKUSDT lifecycle and Hyperliquid admission audit | working tree | local RED/GREEN: closure mapping, empty-balance cache, unavailable admission, and cached-failure recovery `4 passed`; broader regression pending | V1 lifecycle closure table | `entry.pending_registered` is pending lifecycle state; owner and venue events are diagnostics. Hyperliquid remains fail-closed on unavailable balance; its empty response now retains the failure cause and a later successful refresh emits a rate-limited recovery event with a truncated prior-error field. No retry, threshold, execution, close, or recovery policy was relaxed. |
