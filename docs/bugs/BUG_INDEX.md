...
`orderId` scoping are covered in the same production-path matrix. Production
then automatically reconciled exactly one COTI and one ONG event with the
recorded exchange IDs and complete fees; all owners/debts cleared while all
seven venues remained flat/no-order. The repair is closed and accounting-only;
see [CL-123](daily/2026-08-22.md#cluster-cl-123-automatic-historical-close-evidence-exact-recheck).

CL-124 startup/Local-L2 lifecycle diagnostic mapping, 2026-08-22: the first
post-CL-123 diagnosis exposed normal startup event kinds as unmapped. An
exact-only attempt mapped the initial three but the next natural window exposed
nine more Local-L2 kinds, proving enumeration was not the shared boundary.
The closed root repair preserves exact phase start/complete as runtime progress
and assigns the existing account-fee and Local-L2/readiness event families to
their shared runtime-progress/diagnostic phases. Production `67e5c66b` now
reports `unmapped_event_kinds=[]` with the natural family present, green health,
zero blockers, and flat/no-order exchange truth; see
[CL-124](daily/2026-08-22.md#cluster-cl-124-startup-local-l2-lifecycle-diagnostic-mapping).
...
truth-unavailable close reconciliation must block its matching pair; terminal
`evidence_debt` with high-confidence flat/no-order truth must remain warning
visible without disabling that pair forever.  RecoveryLedger now owns that
single distinction and the duplicate dispatch gate is removed.  The same
contract review found three independent defects: ordinary health hid the
background owner to make deploy admission green; targeted OI reused a broad
four-endpoint ticker fetch and collapsed HTTP 429 into timeout; and final
dispatch gate reasons disappeared into `no_entry_dispatched`.  The worktree
repairs split ordinary health from explicit deployment acceptance, make
targeted OI issue only the symbol OI request while preserving 429/retry-after,
and carry exact dispatch blockers into the existing scan diagnostic.  At that
review point these were **local-green, not deployed or closed**; the combined
full validation profile passed 1,571 tests and the complete suite passed 4,386
tests with 9 skipped.  The ledger cache is now invalidated against the owner semantics at
ledger-build time, covering prior-lane additions as well as status transitions
and removals.  Historical billing evidence is not auto-repaired,
and the observed zero-order window still mostly reflects strategy thresholds
rather than one global execution blocker.  See
[CL-119](daily/2026-08-22.md#cluster-cl-119-terminal-close-debt-entry-gate-semantics),
[CL-120](daily/2026-08-22.md#cluster-cl-120-background-debt-health-and-deploy-admission-split),
[CL-121](daily/2026-08-22.md#cluster-cl-121-targeted-oi-refresh-rate-limit-causality),
and [CL-122](daily/2026-08-22.md#cluster-cl-122-final-entry-dispatch-blocker-observability).

Post-deploy circuit-breaker follow-up: `2377193c` proved the CL-120 split in
production and deployed CL-121/122 awaiting natural samples, but it did not
close CL-119.  The live closure table still marked COTI/ONG debt rows blocking
because the first CL-119 predicate consumed diagnostics keys that the runtime
symbol-truth producer never emits.  The second repair moves both payload shapes
behind one recovery-core exact-pair flatness predicate and adds the real
collector → ledger → closure → candidate-gate regression.  This recurrence is
`runtime-symbol-truth-schema-mismatch`, not a new exchange position or order.

The next deployment, `3fc91f68`, exposed the remaining reachability edge.
The exact-pair predicate was present, but a normal `running` restart built its
targeted recovery symbol set from open positions, pending entries/closes,
passive closes, residuals, and journal owners while omitting
`pending_close_reconciliations`.  Therefore COTI/ONG never reached the runtime
truth collector and their cached rows stayed fail-closed.  V1's
`position_management_symbols()` includes pending close reconciliations.  The
follow-up adds that owner to the existing shared startup symbol projection,
including legacy `position_snapshot.symbol`, and covers the persisted
...
The earlier run `lightfee-1787395032184-4057959` also supplied CL-122's missing
natural production sample.  ONG was selected once and dispatched zero; the
same `scan.no_entry_diagnostics` event reported
`entry_dispatch_blocked_counts.recovery_ledger_blocked=1`,
`entry_final_gate_blocked_counts.recovery_ledger_blocked=1`, and
`candidate_stage_blocked_counts.entry_dispatch=1`.  This proves the shared
dispatch counter preserved the exact final blocker without changing admission.
CL-122 is closed; CL-119's later repair removed that false gate itself.
...
converted that warning into a hard failure. The health boundary now permits a
deploy only when the existing high-confidence flat/no-order checks pass and the
only owner is that background accounting task; it leaves the task visible and
...
`code="00000"` with `data.entrustedList=null`. The shared V2 parser required a
list and consequently kept the recovery owner risk-only. The minimal root fix
preserves fail-closed behavior for missing/non-success business codes and
unknown nullable shapes, but normalizes this exact successful Classic response
to an empty collection in the existing shared parser. Direct passive-close,
startup-recovery, and residual-repair fallbacks pass the raw venue-success
context; no screening, trading, or recovery workflow is added. Focused
production-path and parser regressions pass (`135`), transport regression
passes (`25`), and the OPGUSDT incident replay passes (`8`). A stalled broader
close-profile probe is not claimed as a full suite. Cloud `c48f59a` then
fast-forwarded and restarted cleanly: its normal recovery retry emitted the
flat-probe/fallback/`recovery.flat` terminal sequence, durable and compact
state cleared `CLUSDT`, and a high-confidence post-deploy probe found both
OKX and Bitget flat with no open order. This target closure is verified. The
global acceptance gate remains non-green for five V1-compatible background
accounting evidence debts (four historical plus this identity-less close),
and `runtime.passive_close_recovery_result` remains an unmapped diagnostic
event; neither is evidence of remaining CLUSDT exposure. See
[daily/2026-08-12.md#cluster-cl-100-bitget-success-null-open-order-truth](daily/2026-08-12.md#cluster-cl-100-bitget-success-null-open-order-truth).
...
and one historical Aster `-1111` precision rejection. CL-097 closes the three
local root causes at shared boundaries: dynamic Aster order preparation,
passive-close terminal status policy, and the canonical
`pending_close_owner_count` projection. Production health cannot report a
pending close owner as green, and explain output exposes the same projection;
incomplete or non-finite exchangeInfo filters are unavailable rule evidence,
not static values masquerading as exchange metadata.
Legacy reconciliation records missing venue identity and stale Aster/TRUTH
quotes remain evidence-only watch items; no guessed data repair or strategy
relaxation is allowed. The correction is local and deploy pending. See
[daily/2026-08-09.md#cluster-cl-097-live-order-preparation-passive-close-terminality-and-owner-count](daily/2026-08-09.md#cluster-cl-097-live-order-preparation-passive-close-terminality-and-owner-count).
...
|---|---|---:|---|---|---|---|---|---|---|---|
| [CL-124-startup-local-l2-lifecycle-diagnostic-mapping](daily/2026-08-22.md#cluster-cl-124-startup-local-l2-lifecycle-diagnostic-mapping) | closed; `67e5c66b` deployed and production verified | low | `diagnose-live`, `v1-lifecycle-closure`, `local-l2`, `startup-observability` | `event-family-missing-from-lifecycle-projection + exact-sample-fix-missed-shared-prefix-family` | 2026-08-22 | two consecutive natural post-deploy startup/Local-L2 event windows | `67e5c66b` | production-shaped RED/GREEN plus lifecycle/diagnose `135 passed`; complete suite `4412 passed, 9 skipped` | exact phase events retain `RUNTIME_PROGRESS`; existing account-fee and Local-L2/readiness prefix owners provide the shared classification | Production natural window has `unmapped_event_kinds=[]`, zero warnings/blockers/order errors, `RUNNING_CLEAN`, and seven-venue flat/no-order truth. No trading semantics changed. |
| [CL-123-automatic-historical-close-evidence-exact-recheck](daily/2026-08-22.md#cluster-cl-123-automatic-historical-close-evidence-exact-recheck) | closed; deployed in `62474b39`, retained in `67e5c66b`, production verified | high | `close-runtime`, `venue-binance`, `venue-bybit`, `billing`, `historical-evidence`, `exchange-docs` | `offline-candidate-workflow-unreachable-from-runtime + evidence-debt-unconditional-skip + stale-known-cid-short-circuit` | 2026-08-22 | COTI/ONG persisted owners, exact read-only Binance/Bybit order/execution/fee history, V1 exact-identity semantics, official exchange API contracts, and deployed `exit.reconciled` evidence | `194861c` | real runtime-to-adapter regressions cover COTI/ONG, stale CID fallback, Hedge/One-way, pagination, ambiguity, fees, exact execution identity scoping, and terminal truth (dedicated `11 passed`; related selection `191 passed, 338 deselected`); complete suite `4411 passed, 9 skipped`; full profile 10/10, `1571` tests | unique bounded candidate followed by existing exact order/trade/execution recheck and billing gate | Accounting-only: no order action. COTI/ONG each reconciled once with exact IDs and complete fees; owner/debt counts are zero, restart produced no duplicate, and seven-venue truth remains flat/no-order. |
...
| [CL-119-terminal-close-debt-entry-gate-semantics](daily/2026-08-22.md#cluster-cl-119-terminal-close-debt-entry-gate-semantics) | closed; `0474ca28` deployed and production verified | critical | `recovery-ledger`, `recovery-core`, `startup-recovery`, `entry-dispatch`, `v1-parity`, `close-accounting` | `terminal-evidence-debt-permanent-pair-block + active-close-reconciliation-nonblocking-ledger + duplicate-gate-owner + stale-owner-cache + runtime-symbol-truth-schema-mismatch + startup-close-debt-symbol-omission + symbol-open-order-noncontract-adapter-method` | 2026-08-22 | historical COTI/ONG debt plus three production RED closure reviews | `0474ca28` | persisted official-contract startup path; focused `2`, adjacent `343`, complete `4398`, full profile `1571`; production COTI/ONG rows both nonblocking with all seven venues flat/no-order | RecoveryLedger sole pair gate; shared recovery truth predicate, startup owner-symbol projection, order-operation contract/parser, and ledger-build owner fingerprint | Active/unknown/target-truth-gap work blocks; terminal exact-pair flat/no-order debt remains warning-visible and nonblocking. Historical debt/state is unchanged. |
| [CL-100-bitget-success-null-open-order-truth](daily/2026-08-12.md#cluster-cl-100-bitget-success-null-open-order-truth) | deployed; target cloud verified; global accounting acceptance non-green | critical | `exchange-truth`, `passive-close`, `startup-recovery`, `residual-repair`, `v1-parity` | `bitget.open-orders.success-null-treated-untrusted` | 2026-08-12 | read-only CLUSDT OKX/Bitget recovery audit at `8d8a56b` | `c48f59a` | local production-path `135 passed`; transport `25 passed`; incident replay `8 passed`; cloud high-confidence flat/no-open-orders | [passive-close-terminal-flatness](cards/passive-close-terminal-flatness.md) | Exact raw Bitget Classic success `code=00000` plus `data.entrustedList=null` now means no pending order; target recovery cleared normally. Five background accounting evidence debts and one unmapped diagnostic remain separate non-green observability/accounting work. |
| [CL-097-live-order-preparation-passive-close-terminality-and-owner-count](daily/2026-08-09.md#cluster-cl-097-live-order-preparation-passive-close-terminality-and-owner-count) | local fixed; deploy pending; evidence-only watch items retained | critical | `aster-order-preparation`, `passive-close`, `recovery-owner`, `diagnose-live`, `production-health`, `v1-lifecycle`, `explain` | `aster-precision-bypass + rejected-zero-fill-passive-close-loop + passive-owner-count-fallback` | 2026-08-09 | read-only cloud audit at `19470e0` | working tree | local regressions GREEN; cloud verification pending | V1 dynamic symbol rules, terminal close truth, and shared recovery-owner projection | Aster private and generic order routes share one exchange-rule preparation contract; incomplete exchangeInfo is fail-closed; rejected/expired zero-fill maker statuses use the existing passive phase budget; canonical ordinary + passive owner counts block health green and acceptance. Legacy identity-less reconciliation records and stale quotes remain fail-closed/watch-only. |
| [CL-095-passive-close-billing-recovery-symbol-and-joint-entry-grid](daily/2026-08-04.md#cluster-cl-095-passive-close-billing-recovery-symbol-and-joint-entry-grid) | base deployed `ec1d305`; restart-evidence correction local, deploy pending | critical | `passive-close`, `trade-ledger`, `recovery-state`, `recovery-owner`, `entry-planner`, `v1-lifecycle` | `passive-close-terminal-bill-missing + passive-close-restart-order-identity-loss + passive-close-restart-escalation-budget-loss + venue-wire-symbol-owner-mismatch + joint-entry-grid-hedge-undercut + okx-minsz-hedge-undercut` | 2026-08-04 | read-only post-deploy lifecycle audit | deployed `ec1d305`; working tree correction | local regression `346 passed`; full `4098 passed, 9 skipped, 1 warning`: snapshot → restart preserves close evidence and fallback budget; legacy evidence remains provisional | V1 hedgeability planner and passive-close terminal contract | Filled passive closes write one projected terminal bill with fill evidence. The deployed recovery revealed that per-order close evidence and behavior-affecting retry budget were not snapshot-persistent; the correction restores both without guessing PnL/order identity. Known venue wire symbols do not create false orphan ownership; maker/hedge quantities share an executable joint grid and OKX `ctVal × minSz` minimum before maker submission. |
| [CL-094-post-deploy-bankusdt-lifecycle-closure-and-hyperliquid-recovery-observability](daily/2026-08-04.md#cluster-cl-094-post-deploy-bankusdt-lifecycle-closure-and-hyperliquid-recovery-observability) | base deployed `ec1d305`; recovery-fail-closed mapping local, deploy pending | high | `v1-lifecycle-closure`, `entry-admission`, `production-observability`, `v1-parity` | `post-deploy-bankusdt-lifecycle-closure-unmapped + recovery-fail-closed-unmapped + hyperliquid-entry-balance-recovery-evidence` | 2026-08-04 | production BANKUSDT lifecycle and Hyperliquid admission audit | deployed `ec1d305`; working tree correction | local lifecycle/state/passive regression `177 passed` | V1 lifecycle closure table | `runtime.recovery_fail_closed` is now explicitly `RECOVERY_TRUTH`; pending, owner, and venue classifications remain unchanged. Hyperliquid remains fail-closed on unavailable balance. No retry, threshold, execution, close, or recovery policy was relaxed. |
| [CL-093-confirmed-replay-fill-owner-terminalization](daily/2026-08-04.md#cluster-cl-093-confirmed-replay-fill-owner-terminalization) | deployed; cloud health verified; behavior watch | critical | `pending-entry`, `startup-recovery`, `entry-reconciliation`, `v1-parity`, `lifecycle` | `pending-entry.confirmed-positive-fill-removal-before-finalization` | 2026-08-04 | static audit plus deterministic local replay | `3c42aea` | local: targeted startup/normal full-fill and partial-fill regression `4 passed`; full suite `4082 passed, 9 skipped, 1 warning`; cloud manifest/services/diagnose green | [pending-entry-terminality-live-truth](cards/pending-entry-terminality-live-truth.md) | `uncertain_outcome` is no longer a deletion authority: confirmed partial/full fills reconcile exchange truth and finalize/residual before owner removal; a fresh production replay sample remains watch-only evidence. |
| [CL-077-home-lifecycle-quick-flat-recurrence](daily/2026-06-13.md#cluster-cl-077-home-lifecycle-quick-flat-recurrence) | local fixed; deploy pending | critical | `live-runtime`, `pending-entry`, `recovery-core`, `diagnose-live`, `offline-analysis`, `v1-lifecycle-closure`, `production-health` | `HOMEUSDT-entry-opened-then-recovery-flat-quick-flat + positive-fill-live-flat-terminality-conflict + scattered-single-point-closure-recurrence` | 2026-06-13 | production rotated-log HOMEUSDT review after CL-076 cloud closure | working tree | 2026-06-13 local: HOME quick-flat replay RED/GREEN, positive-fill/live-flat runtime RED/GREEN, diagnose gate RED/GREEN, terminalizer/closure regression `28 passed`; broader regression pending. | `HOME lifecycle contract recurrence over CL-064/066/067/076 single-point closures` | Production HOMEUSDT showed 27 `entry.opened` lifecycles and no clean normal open/close; every lifecycle carried recovery, drift, residual, one-sided flatten, missing L2/tick, or exchange-error reconciliation evidence. The local fix upgrades shared lifecycle semantics: quick-flat counting now includes `recovery.flat` and `runtime.position_lifecycle_terminal`, live positive-fill finalization requires non-flat live position truth before `entry.opened`, and the V1 lifecycle closure table uses the same positive-fill/live-truth terminalizer. No HOME freeze, order/cancel, close executor, residual submission, sizing, strategy, OI/liquidity, WS-BBO/Local-L2, credential, or production config change. |
| [CL-076-v1-lifecycle-closure-table-runtime-gate-release-unification](daily/2026-06-13.md#cluster-cl-076-v1-lifecycle-closure-table-runtime-gaterelease-unification) | local fixed; deploy pending | critical | `live-runtime`, `v1-parity`, `pending-entry`, `passive-close`, `residual-repair`, `recovery-core`, `current-state`, `diagnose-live`, `production-health` | `v1-lifecycle-closure-table-runtime-gate-release-unification + scattered-pending-passive-residual-recovery-semantics` | 2026-06-13 | local audit after recent WS-BBO/pending/passive/residual/recovery fixes | `abe7605` | 2026-06-13 local: closure-table suite `16 passed`; related diagnose/production-health/snapshot/startup/entry/passive regression set `551 passed`; compileall passed; diff-check passed; GitNexus `detect_changes --repo LightFeeV2 --scope all` low risk with affected processes `0`, with runtime large-file indexing caveat. | `V1LifecycleClosureTable as runtime/diagnose/health semantic authority` | V2 now maps entry quote lease, pending entry, open position, passive close, residual repair, recovery truth, and runtime progress into one V1 semantic closure surface. Runtime entry gate reads the table summary, recovery block/clear events include closure identity, and pending/passive/residual release paths emit `closure_decision_id`. The fix reuses runtime-owned recovery ledger/core results to avoid owner drift and does not change trading aggressiveness, strategy thresholds, WS-BBO budget/TTL, Local-L2 opt-in, order submit/cancel, close execution, residual repair submission, or recovery flatten policy. |
