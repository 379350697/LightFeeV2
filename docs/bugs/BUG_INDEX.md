...
|---|---|---:|---|---|---|---|---|---|---|---|
| [CL-122-final-entry-dispatch-blocker-observability](daily/2026-08-22.md#cluster-cl-122-final-entry-dispatch-blocker-observability) | local-green; deploy pending | high | `live-runtime`, `entry-dispatch`, `production-observability` | `selected-candidate-final-gate-reason-collapsed-to-no-entry-dispatched` | 2026-08-22 | post-deploy zero-entry log/code review at `a5ef64a` | working tree | selected-dispatch Counter and real `scan.no_entry_diagnostics` payload regression green | existing entry-gate reasons and bounded scan diagnostics | Final gate order/outcomes are unchanged; exact dispatch blockers are now separated from selection blockers and merged into the final-gate diagnostic view. |
| [CL-121-targeted-oi-refresh-rate-limit-causality](daily/2026-08-22.md#cluster-cl-121-targeted-oi-refresh-rate-limit-causality) | local-green; deploy pending | high | `market-data`, `entry-readiness`, `venue-binance`, `venue-aster`, `rate-limit-observability` | `targeted-oi-full-ticker-fanout + http-429-collapsed-to-timeout` | 2026-08-22 | production Aster 429 plus code-path review at `a5ef64a` | working tree | direct OI-only request, mark-price reuse, shared 429/retry-after classifier, stale-counter reset, and runtime fail-closed regressions green | official per-symbol OI endpoint and existing client limiter/cache | Candidate OI refresh no longer calls book/premium/24h endpoints and preserves typed rate-limit causality; unavailable OI still blocks entry. |
| [CL-120-background-debt-health-and-deploy-admission-split](daily/2026-08-22.md#cluster-cl-120-background-debt-health-and-deploy-admission-split) | local-green; deploy pending | high | `production-health`, `deployment`, `recovery-accounting` | `background-close-owner-hidden-to-make-health-ok` | 2026-08-22 | CL-116/118 post-deploy semantic review at `a5ef64a` | working tree | ordinary warning/non-green plus exact explicit deployment acceptance and generated-script regressions green | shared recovery evidence plus explicit verifier policy boundary | Background debt stays visible in ordinary health; only the proven-flat sole-warning shape can pass explicit deployment admission. |
| [CL-119-terminal-close-debt-entry-gate-semantics](daily/2026-08-22.md#cluster-cl-119-terminal-close-debt-entry-gate-semantics) | local-green; redeploy pending | critical | `recovery-ledger`, `recovery-core`, `startup-recovery`, `entry-dispatch`, `v1-parity`, `close-accounting` | `terminal-evidence-debt-permanent-pair-block + active-close-reconciliation-nonblocking-ledger + duplicate-gate-owner + stale-owner-cache + runtime-symbol-truth-schema-mismatch + startup-close-debt-symbol-omission` | 2026-08-22 | historical COTI/ONG debt plus post-`2377193c` and post-`3fc91f68` production closure reviews | working tree on `3fc91f68` | persisted `runtime.start()` → symbol collector → ledger → closure → candidate gate; exact release, top-level/snapshot discovery, complementary fail-closed matrix; adjacent `319`, close `444`, complete `4398`, full profile `1571` passed | RecoveryLedger sole pair gate; recovery-core account/diagnostics/symbol truth predicate; shared startup owner-symbol projection; ledger-build owner fingerprint | The second deployment proved the predicate but exposed missing startup reachability. Active/unknown/target-truth-gap work blocks; terminal exact-pair flat/no-order debt is warning-visible and nonblocking once every persisted reconciliation symbol is probed. Historical debt/state is unchanged. |
