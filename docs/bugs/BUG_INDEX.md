...
`runtime_progress` exposed through production health and diagnose. Focused
local verification passed (`125 passed`), compileall and diff-check passed, and
GitNexus detect-changes reported high risk because the intended change touches
`LiveRuntime.run_loop` and current-state export paths. Full evidence is
recorded in
...
|---|---|---:|---|---|---|---|---|---|---|---|
| [CL-074-runtime-heartbeat-health-gate-exporter-only-false-progress](daily/2026-06-12.md#cluster-cl-074-runtime-heartbeat-health-gate-exporter-only-false-progress) | local fixed; deploy pending | high | `live-runtime`, `current-state`, `production-health`, `diagnose-live`, `production-observability` | `runtime-heartbeat-health-gate-exporter-only-false-progress` | 2026-06-12 | post-heartbeat/health-gate review after repeated `live_tick_stale` surface fixes | working tree | 2026-06-12 local: focused production-health/runtime-smoke/diagnose suite `125 passed`; compileall passed; diff-check passed; GitNexus detect-changes high due intended `LiveRuntime.run_loop` and current-state export impact. | `Runtime lane progress schema plus exporter-only health fingerprint` | Current-state exporter freshness no longer proves runtime progress. `run_loop()` records lane-level business progress and bounded active lanes; heartbeat exports snapshots without advancing `last_lane_progress_ms`. Production health suppresses `live_tick_stale` only with recent `last_scan`, recent runtime lane progress, or non-overdue bounded active lane plus flat exchange truth; fresh exporter-only snapshots keep stale tick and add `exporter_only_progress`. Diagnose state consistency and production acceptance gate expose the same `runtime_progress`. |
