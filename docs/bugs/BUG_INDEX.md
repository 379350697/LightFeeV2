...
| [CL-119-terminal-close-debt-entry-gate-semantics](daily/2026-08-22.md#cluster-cl-119-terminal-close-debt-entry-gate-semantics) | local-green; redeploy pending | critical | `recovery-ledger`, `recovery-core`, `entry-dispatch`, `v1-parity`, `close-accounting` | `terminal-evidence-debt-permanent-pair-block + active-close-reconciliation-nonblocking-ledger + duplicate-gate-owner + stale-owner-cache + runtime-symbol-truth-schema-mismatch` | 2026-08-22 | historical COTI/ONG debt plus post-`2377193c` production closure-table review | working tree | actual runtime symbol-truth producer → ledger → closure → candidate gate; exact release, complementary fail-closed matrix, unrelated-probe isolation; adjacent `402`, close `444`, complete `4396`, full profile `1571` passed | RecoveryLedger sole pair gate; recovery-core account/diagnostics/symbol truth predicate; ledger-build owner fingerprint | First deployment exposed a producer/consumer schema drift. Active/unknown/target-truth-gap work blocks; terminal exact-pair flat/no-order debt is warning-visible and nonblocking. Historical debt/state is unchanged. |
