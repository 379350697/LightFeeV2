...
close-profile probe is not claimed as a full suite. The correction is local
and deploy pending, therefore **implemented but not closed** until the normal
recovery retry clears only against fresh flat truth. See
...
|---|---|---:|---|---|---|---|---|---|---|---|
| [CL-100-bitget-success-null-open-order-truth](daily/2026-08-12.md#cluster-cl-100-bitget-success-null-open-order-truth) | local green; deploy pending | critical | `exchange-truth`, `passive-close`, `startup-recovery`, `residual-repair`, `v1-parity` | `bitget.open-orders.success-null-treated-untrusted` | 2026-08-12 | read-only CLUSDT OKX/Bitget recovery audit at `8d8a56b` | working tree | production-path `135 passed`; transport `25 passed`; incident replay `8 passed` | [passive-close-terminal-flatness](cards/passive-close-terminal-flatness.md) | Exact raw Bitget Classic success `code=00000` plus `data.entrustedList=null` now means no pending order; all missing/non-success/unknown envelopes stay fail-closed. |
