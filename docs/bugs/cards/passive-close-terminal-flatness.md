...
| 2026-06-20 | Resolved close artifacts become historical diagnostic evidence | local green follow-up | `classify_noise_visibility(...)` and root `diagnostic_noise_summary` separate resolved ACK-only / duplicate-client / reduce-only / post-only close artifacts from current blockers after exchange flat + no open orders. `short_window_warning_details.passive_close_truth_gap` now counts only unresolved/current truth gaps. Current single-leg exposure, ownerless open orders, untrusted truth, and unresolved reconciliation gaps remain blocking and are not downgraded. |
