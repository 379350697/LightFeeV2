...
| 2026-07-03 | incomplete legacy `exit.closed` before terminal-flat truth | local verified; deploy pending | Incomplete legacy `exit.closed` rows with one close leg zero or uncertain no longer outrank later terminal-flat exchange truth in the unified ledger. They remain excluded from complete PnL samples, but diagnose moves them to `ledger_terminal_flat_legacy_inferred_*` instead of keeping `close_truth_gap_legacy_inferred`. | [daily/2026-07-03.md#cluster-cl-150---incomplete-exitclosed-outranked-terminal-flat-ledger-truth](../daily/2026-07-03.md#cluster-cl-150---incomplete-exitclosed-outranked-terminal-flat-ledger-truth) |
