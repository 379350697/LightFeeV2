...
fingerprint visible unless the ledger has terminal live-flat exchange truth. If
the current event window only retains terminal exchange truth with
positions-flat and open-orders-flat, classify it as
`terminal_flat_exchange_truth_accounting_gap`, report it under
`ledger_terminal_flat_legacy_inferred_*`, and keep it out of complete-PnL
samples until exchange fill statements are present. Legacy `order.filled`
events do not always carry `phase/source/tradeSide`; venue-normalized
`leg+side` is enough to infer close phase (`long+sell`, `short+buy`) or open
phase (`long+buy`, `short+sell`).
...
| 2026-07-01 | close zero-fill diagnostic semantic split | `a9269db` deployed/cloud verified | CL-143 splits passive-close maker zero-fill into normal maker polling, resolved-after-zero-fill, unproductive zero-fill, and maker-viability rejected buckets. Later terminally resolved maker waiting no longer appears as close-cost inefficiency, while no-price/unmaintainable/fallback shapes remain visible. The split is diagnostic only and preserves terminal flat/open-order proof. |
| 2026-07-01 | ACK-only close truth-gap explicit lifecycle follow-up | local verified; deploy pending | CL-144 routes aggressive/duplicate ACK-only close uncertainty through explicit accepted-order truth-gap registration and resolution. Order-detail fill proof emits `resolution_status=filled`; live-flat closure requires live position flat plus open-orders-empty proof; open-order/truth gaps retain. Legacy inferred gaps remain nonblocking audit debt only for old logs. See [daily/2026-07-01.md#cluster-cl-144---ack-only-close-truth-gap-explicit-lifecycle-follow-up](../daily/2026-07-01.md#cluster-cl-144---ack-only-close-truth-gap-explicit-lifecycle-follow-up). |
| 2026-07-25 | Historical malformed pending-close reconciliation records | current exposure closed; noise reduction open | CL-159 observed historical records without complete `position_snapshot_venues`, producing `reconciliation.pending_close_reconciliation_invalid`. Current all-venue truth was flat/no-open-orders and the reconciliation queue drained to zero. Archive/normalize only behind fresh relevant-venue flat truth; absent local owner state is never deletion authority. See [daily/2026-07-25.md#issue-cl-159-f-historical-invalid-pending-close-reconciliation-noise](../daily/2026-07-25.md#issue-cl-159-f-historical-invalid-pending-close-reconciliation-noise). |
...
| 2026-07-03 | legacy close gap consumed by exchange-truth ledger | `2503399`/`0858837` deployed/cloud verified | Legacy `order.filled leg/side` close events feed the ledger, ledger-complete legacy truth gaps move to `ledger_closed_legacy_inferred_*`, while missing exchange-fill evidence stays accounting evidence debt instead of live-risk residue. | [daily/2026-07-03.md#cluster-cl-147---diagnose-consumes-exchange-truth-ledger-for-legacy-close-gaps](../daily/2026-07-03.md#cluster-cl-147---diagnose-consumes-exchange-truth-ledger-for-legacy-close-gaps) |
| 2026-07-03 | terminal-flat-only legacy close evidence gap | `0858837` deployed/cloud verified | Terminal exchange truth with flat positions/open-orders moves to `ledger_terminal_flat_legacy_inferred_*` or clears stale close-gap noise without pretending to be complete accounting/PnL truth; the later full-history gate failure was separate CL-149 residual history drift. | [daily/2026-07-03.md#cluster-cl-148---terminal-flat-legacy-close-gap-into-exchange-truth-ledger](../daily/2026-07-03.md#cluster-cl-148---terminal-flat-legacy-close-gap-into-exchange-truth-ledger) |
| 2026-07-03 | incomplete legacy `exit.closed` before terminal-flat truth | `ab91de9` deployed/cloud verified | Incomplete legacy `exit.closed` rows with one close leg zero or uncertain no longer outrank later terminal-flat exchange truth in the unified ledger. They remain excluded from complete PnL samples, but explicit 7-venue cloud diagnose now moves them to `ledger_terminal_flat_legacy_inferred_*` with `legacy_inferred_count=0` and no `close_truth_gap_legacy_inferred` fingerprint. | [daily/2026-07-03.md#cluster-cl-150---incomplete-exitclosed-outranked-terminal-flat-ledger-truth](../daily/2026-07-03.md#cluster-cl-150---incomplete-exitclosed-outranked-terminal-flat-ledger-truth) |
| 2026-07-05 | pending-close dedupe and venue order-truth compatibility | `4546891` deployed/cloud verified | Duplicate pending-close enqueue preserves owner metadata while merging statement/order candidates, retained accounting-only retries do not duplicate `exit.reconciled`, and Bybit/Bitget omit empty historical-window kwargs for plain order/CID truth lookup. Cloud proof passed manifest/singleton/services and explicit 7-venue since-deploy diagnose with exchange flat/no-open-orders and no pending close reconciliation. | [daily/2026-07-05.md#cluster-cl-153---pending-close-dedupe-and-venue-order-truth-compatibility](../daily/2026-07-05.md#cluster-cl-153---pending-close-dedupe-and-venue-order-truth-compatibility) |
