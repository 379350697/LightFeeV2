...
| 2026-06-24 | Bitget passive cancel contract family params | `9babfac` deployed/cloud health verified; residual lifecycle watch | CL-114 keeps `owned_pending_entry_live_conflict` as the correct blocker but fixes the owner cleanup transport path: Bitget passive cancel now resolves Classic vs UTA account family and sends the family-required Classic `productType/marginCoin/symbol` or UTA `category` fields. Cloud post-deploy health is running with exchange truth flat/no-open-orders and no fresh `productType cannot be empty` cancel errors; since-deploy diagnose still reports HOMEUSDT `residual_events_present` as diagnostic residue, not a current live exposure. This is a venue-contract drift, not a relaxation of pending-entry terminality or live-truth gates. See [daily/2026-06-24.md#cluster-cl-114---bitget-passive-cancel-contract-family-params](../daily/2026-06-24.md#cluster-cl-114---bitget-passive-cancel-contract-family-params). |
