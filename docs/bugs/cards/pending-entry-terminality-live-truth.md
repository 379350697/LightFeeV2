...
| 2026-08-30 | Matched ZKP hedge remained `risk_only` beside historical COTI close debt | deployed-awaiting-verification; `2111e36` through `fe821142` | The first account-truth release attempt (`8a2def63`) correctly matched the normal hedge, but the preceding "only close reconciliation" predicate rejected every nonempty `local_open_positions` list. V1 permits confirmed active positions to keep close accounting in the background. The existing core predicate now accepts local positions only when complete account truth exactly matches every local long/short pair and contains no open order; incomplete, one-leg, wrong-side, wrong-quantity, or order-bearing truth stays blocked. The production-shaped terminal pending removal regression carries an unrelated final `evidence_debt`; full pytest is green. Post-deploy state is running with that debt as background work; a new natural matched-pair observation remains required. |
...
| 2026-08-30 | `ZKPUSDT` OKX/Binance normal hedge + historical COTI close debt | `2111e36` through `fe821142` | deployed-awaiting natural matched-pair observation | The live entry was quantity- and side-matched with no open orders, but the prior `unpaired_live_position` latch stayed `risk_only` solely because an unrelated terminal close debt existed. |
...
30. Distinguish retryable/unknown pending-close work from terminal
    ...
    orders empty.  In both cases the close owner remains visible to ordinary
    health until exact accounting evidence settles it.
