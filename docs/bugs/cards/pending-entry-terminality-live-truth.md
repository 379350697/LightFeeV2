...
| 2026-06-07 | JTOUSDT maker zero-fill weak terminal evidence | locally verified; pending cloud deploy | Mapped to `PE-02`, `PE-13`, and `PE-18`. Runtime had terminal no-fill order reconciliation and live position `0`, but open-order truth was unavailable/eventually inconsistent. V2 compressed that evidence gap into `available=True / no live artifact`, emitted `entry.passive_unfilled`, and removed pending ownership. The fix keeps unavailable open-order truth, live-position truth, or live maker order reference as deferred terminalizer input, retains pending with backoff, and preserves maker order id/client id so later open-order truth routes to owned cancel/reconcile instead of `orphan_maker_order`; flat reconcile and stale abandon paths now use the same strong maker-terminal evidence rule. This is a weak terminal-evidence bug in pending-entry zero-fill finalization, not a RecoveryLedger cleanup bug. Local verification passed targeted pending-entry/recovery/passive suites, compileall, diff-check, and full pytest `3685 passed`, `9 skipped`, `1 warning`. |
