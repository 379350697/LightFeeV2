...
| 2026-09-01 | Gate/Bitget sidecar parsed stale aliases and used local `now` as the funding schedule | deployed-awaiting-verification at `12d8e038` | Production showed Bitget `bidPr`/`askPr` as zeroed quotes and both venues' first funding time within one snapshot interval of observation. The deployed sidecar now has 93/93 usable Bitget and 90/90 usable Gate rows with nonzero quotes and future exchange schedules, with zero `CLOSE_WAIT`. Restore documented bulk metadata parsing, seconds-to-ms Gate conversion, existing bounded Gate cache, and fail-closed `0` on absent/stale metadata. Do not cherry-pick a divergent branch wholesale, add a polling loop, or relax the V1 funding window. |
