...
not terminal repair failures. The same tick must reconcile accepted order truth
before another reduce-only can be submitted: execution/fill truth wins first;
live-flat plus trusted empty open-order truth completes and releases the gate;
open order present or unavailable truth keeps the existing pending repair task,
...
| 2026-07-03 | Completed historical residuals with active positions | `25b59e0` deployed/cloud verified | CL-149 stops full-history explicit venue diagnose from treating already completed old residual events as current blockers when there are current owner-managed active positions. The gate exposes `residual_lifecycle_complete=true` and blocks only when local pending residual work or unclosed residual lifecycle keys remain; cloud explicit 7-venue diagnose passed with `residual_lifecycle_complete=true` and no `residual_events_present` blocker. See [daily/2026-07-03.md#cluster-cl-149---completed-historical-residuals-with-active-positions](../daily/2026-07-03.md#cluster-cl-149---completed-historical-residuals-with-active-positions). |
