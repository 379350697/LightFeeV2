...
- `runtime.maker_event_reprice_blocked` with
  `reason=unhedged_single_leg_risk` after a hedge reject or single-leg pending
  exposure.
- Binance maker-event reprice `-2019 Margin is insufficient` after pending
  exposure; classify it as `insufficient_margin_admission_blocked`, not ordinary
  retryable reprice noise.
...
22. If the hedge submit is exchange-rejected after maker exposure and there is
    no accepted hedge order identity, treat it as
    `hedge_rejected_after_maker_exposure` even when the venue body is generic.
    The branch must go to admission cleanup/cooldown, not
    `accepted_order_truth_gap`, and maker-event reprice must remain blocked
    while maker fill outruns hedge fill.
...
| 2026-06-28 | Hyperliquid API-wallet strict-readonly admission closure | local green, deploy pending | `cafdf0a` introduced identity preflight and `7e058b0` fixed wallet-mode source alignment, making the reason accurately surface as `api_wallet_authorization_not_verified_strict_readonly`. Diagnose now separates `account_state_readable`, `signer_matches_account`, and `trading_authorization_trusted`, scans retained logs for canonical Hyperliquid order/open evidence, and entry admission prunes strict-readonly Hyperliquid candidates before real submit while keeping quote/exchange-truth reads enabled. |
...
| 2026-06-26 | Aster advisory lifecycle mapping and card drift | code fix `7351c6f` deployed/cloud verified; final docs sync also deployed | `runtime.entry_admission_headroom_advisory` is mapped to `DIAGNOSTIC_ONLY`; since-deploy diagnose no longer reports it as unmapped residue. This card now records CL-120 scope semantics: endpoint-zero advisory does not block, real `-5018` is `aster:SYMBOL`, and `aster:*` requires account-level or venue-wide evidence. |
| 2026-07-05 | Generic hedge reject after maker exposure and maker-event reprice churn | `4546891` deployed/cloud verified | CL-152 routes unclassified exchange `REJECTED` hedge submit after maker exposure into `hedge_rejected_after_maker_exposure` cleanup/cooldown on both normal and recovery hedge ticks. Rejected no-order hedge attempts no longer become accepted-order truth gaps. Maker-event local L2, WS-BBO, and sidecar branches block `unhedged_single_leg_risk` using V1 `missing_hedge_quantity()` semantics rather than raw maker/hedge counters; Binance `-2019` maps to `insufficient_margin_admission_blocked`. Cloud proof passed manifest/singleton/services and explicit 7-venue since-deploy diagnose with exchange flat/no-open-orders, no pending entries, and no residual work. |
...
| 2026-06-28 | `ESPORTSUSDT` Aster real `-5018` passive submit reject repeated after structured `order.rejected` evidence | `e1ea937` | deployed/cloud verified; `EntryExecutionResult.reject_evidence` now carries adapter `exchange_error` to runtime admission core, arming `aster:SYMBOL` with `source=exchange_5018_fallback`; advisory-zero stays nonblocking | [daily/2026-06-28.md#cluster-cl-132---aster-structured-submit-reject-evidence-to-admission-core](../daily/2026-06-28.md#cluster-cl-132---aster-structured-submit-reject-evidence-to-admission-core) |
| 2026-07-05 | `LABUSDT` Binance maker / Aster hedge, generic Aster V3 400 after maker exposure plus Binance `-2019` maker-event churn | `4546891` | local RED/GREEN covers normal hedge, recovery hedge, and maker-event lanes; cloud 7-venue diagnose passed flat/no-open-orders with no pending entry/order-truth/residual work | [daily/2026-07-05.md#cluster-cl-152---entry-single-leg-risk-invariant-across-hedge-reject-and-maker-reprice](../daily/2026-07-05.md#cluster-cl-152---entry-single-leg-risk-invariant-across-hedge-reject-and-maker-reprice) |
