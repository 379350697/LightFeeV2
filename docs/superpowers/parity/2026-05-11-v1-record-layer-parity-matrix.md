...
| JE-002 | Envelope fields `seq,run_id,ts_ms,kind,payload` match but type divergence: Rust `seq:u64,ts_ms:i64` vs Python `seq:int,ts_ms:int` | `journal_bridge.rs:32-41` | `JournalRecord` | `lightfee/persistence/journal.py:52-58` | `Journal.append` | All journal callers | Type semantic drift (unsigned vs signed for seq) | Documented: Python int (arbitrary precision) covers all V1 integer domains (u64 seq, i64 ts_ms) without range loss | `tests/test_persistence.py` | P2 | fixed |
...
| OL-002 | Rust emits `entry.aborted`, `entry.aborted_failed_pending_retained`; Python has no abort lifecycle records | `entry_sync.rs` | abort paths | `lightfee/engine/entry_sync.py:103-120` | `EntrySyncExecutor.execute` | `LiveRuntime` abort handling | Python drops abort evidence entirely | Add `entry.aborted`, `entry.aborted_failed_pending_retained` journal events | `tests/test_engine_entry_exit.py` | P1 | fixed |
| OL-003 | Rust emits `order.submitted`, `order.filled`, `order.rejected` per-order; Python now uses canonical `order.*` kinds with leg in payload | `entry_sync.rs`, `entry.rs` | order submission paths | `lightfee/engine/entry_sync.py`, `lightfee/engine/close_executor.py` | `_submit_maker`, `_submit_hedge`, `_submit_order`, `_submit_close_leg` | `EntrySyncExecutor.execute`, `CloseExecutor.execute_close` | All entry and exit order kinds unified to canonical `order.submitted`, `order.filled`, `order.rejected`, `order.uncertain` with `leg` discriminator in payload | `tests/test_engine_entry_exit.py` | P1 | fixed |
...
| OL-006 | Rust `exit.partial_closed` payload includes `position_id`, `quantity`, `current_net_quote`, `peak_net_quote`, `funding_captured`, `second_stage_funding_captured`; Python matches core fields | `replay_bridge.rs:373-397` | `update_position_state` | `lightfee/engine/close_executor.py:516-533` | `CloseExecutor._writeback_to_state` (producer) | Replay/Persistence | Partial close payload shape matches; verify `peak_net_quote`, `funding_captured`, `second_stage_funding_captured` are emitted at producer | Emit `exit.partial_closed` at producer with full partial-close fields | `tests/test_persistence_replay.py` | P1 | fixed |
