...
| recovered pending close non-flat/probe-failure diagnostics | FAIL -> GREEN locally; deployed in `6845d9b`; production trigger absent after restart | XCNUSDT recovered tests and diagnose-live Aster/position-id tests |
...
| Medium | Recovered pending close was observable in production without enough structured evidence when it was not yet proven flat/non-flat, especially for Aster pairs and events that only carried the recovered symbol in `position_id`. | XCNUSDT diagnostics tests first failed because no diagnostic event was emitted for one-side nonzero/fetch failure, `diagnose_live` did not target Aster, and symbol filter missed position-id-only events. | GREEN locally; deployed in `6845d9b`; no fresh one-sided/nonfetchable production trigger after restart |
...
Follow-up production read-only acceptance for `live-recovered:XCNUSDT:bybit->aster` showed the deployed live-flat sweep cleared the stale local recovered state: both `live-state.json` and `live-state-current.json` had `open_position_count=0`, `pending_passive_close_count=0`, and `last_error=null`; Bybit and Aster XCNUSDT positions were `0.0`; Bybit and Aster XCNUSDT open order counts were `0`. This is a symbol-specific evidence closure for XCNUSDT, not proof that every old close-parity ledger item is cloud verified. The new diagnostic enhancements were deployed by cloud-side git pull/reset to `6845d9b`; remote compileall passed, live/sidecar restarted active, and post-restart state/log checks remained clean. Because no one-sided/nonfetchable recovered pending close exists after restart, the diagnostic branch is production-deployed but production-trigger-absent; behavioral proof remains the local RED/GREEN tests.
